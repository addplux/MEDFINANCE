    ...
    return (
        <div className="space-y-8 animate-fade-in max-w-6xl mx-auto pb-20">
            ...
            <div className="flex items-center gap-4">
                <button onClick={() => navigate(-1)} className="p-3 hover:bg-bg-tertiary rounded-2xl text-text-tertiary hover:text-text-primary transition-all">
                ...
                <div>
                    ...
                    <p className="text-xs text-text-tertiary mt-1">Define which services this scheme covers and set custom prices per service.</p>
                </div>
            </div>

            {/* Covered Services */}
            <div className="bg-bg-secondary/40 border border-border-color rounded-[2.5rem] shadow-2xl overflow-hidden backdrop-blur-sm">
                <div className="flex items-center justify-between px-6 py-4 border-b border-border-color">
                    <div className="flex items-center gap-3">
                        <CircleCheckBig className="w-5 h-5 text-emerald-400" />
                        <h2 className="text-sm font-bold text-text-primary">Covered Services</h2>
                    ...
                    <button onClick={load} className="p-2 hover:bg-bg-tertiary rounded-lg text-text-tertiary hover:text-text-primary transition-all">
                ...
                {loading ? (
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead>
                                <tr className="border-b border-border-color text-[9px] uppercase tracking-widest text-text-tertiary">
                            ...
                            <tbody className="divide-y divide-border-color">
                                {covered.map(ss => (
                                    <tr key={ss.id} className="hover:bg-bg-secondary/40 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="text-sm font-semibold text-text-primary">{ss.service?.serviceName}</div>
                                            <div className="text-xs text-text-tertiary font-mono">{ss.service?.serviceCode}</div>
                                        </td>
                                        <td className="px-6 py-4">
                                            ...
                                            {editingId === ss.id ? (
                                                <div className="flex items-center gap-2">
                                                    <input
                                                        ...
                                                        className="w-32 px-3 py-1.5 bg-bg-tertiary border border-border-color rounded-lg text-sm text-text-primary focus:outline-none focus:border-primary/50"
                                                        autoFocus
                                                    />
                                                    ...
                                                    <button onClick={() => setEditingId(null)} className="p-1.5 hover:bg-bg-tertiary text-text-tertiary hover:text-text-primary rounded-lg transition-colors">
                                                        <X className="w-4 h-4" />
                                                    </button>
                                                </div>
                                            ) : (
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <button
                                                ...
                                                className="p-1.5 hover:bg-red-500/10 text-text-tertiary hover:text-red-400 rounded-lg transition-colors"
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2 bg-bg-tertiary border border-border-color rounded-xl text-sm text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-accent/50"
                            />
                        </div>
                        <div className="flex gap-2 flex-wrap">
                            {categories.map(c => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-xl text-xs font-semibold capitalize transition-colors ${catFilter === c ? 'bg-accent/20 text-accent border border-accent/30' : 'bg-bg-tertiary text-text-tertiary hover:text-text-primary border border-border-color'}`}
                                >
                                    {c}
                                </button>
                            ))}
                    ...
                    {filteredUncovered.length === 0 ? (
                        ...
                        <div className="divide-y divide-border-color">
                            {filteredUncovered.map(s => (
                                <div key={s.id} className="flex items-center justify-between px-6 py-4 hover:bg-bg-secondary/40 transition-colors">
                                    <div>
                                        <div className="text-sm font-semibold text-text-primary">{s.serviceName}</div>
                                        <div className="flex items-center gap-3 mt-1">
                                            <span className="text-xs font-mono text-text-tertiary">{s.serviceCode}</span>
