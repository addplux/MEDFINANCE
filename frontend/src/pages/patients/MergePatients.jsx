        ...
        return (
            <div className={`p-4 rounded-lg border-2 ${type === 'primary' ? 'border-green-500 bg-green-50' : 'border-red-500 bg-red-50'}`}>
                ...
                <div className="flex gap-4">
                    {patient.photoUrl && (
                        <img src={`${import.meta.env.VITE_API_URL.replace('/api', '')}${patient.photoUrl}?token=${localStorage.getItem('token')}`} alt="Patient" className="w-16 h-16 rounded-full object-cover" />
                    )}
                    <div>
