        ...
        <div className="space-y-5 pb-10">
            ...
            {data && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="card p-4 text-center">
                        <p className="text-xs text-white/50 uppercase tracking-wide">Total Visits</p>
                        <p className="text-3xl font-bold text-white mt-1">{visits.length}</p>
                    </div>
                    <div className="card p-4 text-center">
                        <p className="text-xs text-white/50 uppercase tracking-wide">Visit Types</p>
                        <p className="text-3xl font-bold text-white mt-1">{types.length}</p>
                    </div>
            ...
            {types.length > 1 && (
                <div className="card p-3 flex items-center gap-2 flex-wrap">
                    <span className="text-xs text-white/60 font-medium">Filter by type:</span>
                    <button
                        onClick={() => setTypeFilter('')}
                        className={`px-3 py-1 rounded-full text-xs font-semibold border transition-colors ${typeFilter === '' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white/5 text-white/70 border-white/10 hover:border-indigo-400'}`}
                    >
                        All ({visits.length})
                    </button>
                    {types.map(t => (
                        <button
                            ...
                            className={`px-3 py-1 rounded-full text-xs font-semibold border transition-colors ${typeFilter === t ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white/5 text-white/70 border-white/10 hover:border-indigo-400'}`}
                        >
                    ...
                    <>
                        ...
                        <div className="hidden md:block overflow-x-auto">
                            <table className="w-full text-left">
                                <thead className="text-xs font-bold text-white/60 uppercase bg-white/5 border-b border-white/10">
                                    <tr>
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {filtered.map((visit, i) => {
                                        ...
                                        const statusCls = statusColors[status?.toLowerCase()] || 'text-white/50 bg-white/5';
                                        return (
                                            <tr key={`${visit.visitType}-${visit.id ?? i}`} className="hover:bg-white/5 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-right text-sm font-semibold text-white/90">
                                ...
                                <tfoot className="bg-white/5 border-t-2 border-white/10">
                                    <tr>
                                        <td colSpan="4" className="px-4 py-3 text-sm font-bold text-white/80">
                                            Total ({filtered.length} records)
                                        </td>
                                        <td className="px-4 py-3 text-right text-sm font-bold text-indigo-400">
                            ...
                            {filtered.map((visit, i) => {
                                ...
                                return (
                                    <div key={`${visit.visitType}-${visit.id ?? i}`} className="p-4 space-y-2">
                                        ...
                                        <p className="text-sm text-white/80 truncate">{getDescription(visit)}</p>
