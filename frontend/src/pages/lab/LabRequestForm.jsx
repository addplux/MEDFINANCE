...
import { labAPI, patientAPI, visitAPI } from '../../services/apiService';
            ...
            <div className="p-4 border-b border-white/10 bg-white/[0.02]">
                <div className="flex items-center justify-between mb-2">
                    <h2 className="text-xs font-black uppercase tracking-widest text-white/40 flex items-center gap-2">
                        <Clock className="w-3 h-3" /> Lab Queue
                    </h2>
                ...
                {loading && queue.length === 0 ? (
                    ...
                    <div className="p-8 text-center">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-white/20 leading-relaxed">
                            No patients<br />waiting for lab
                        </p>
                    </div>
                ) : (
                    queue.map((item) => (
...
const LabRequestForm = () => {
    ...
    const fetchInitialData = async () => {
        try {
            const tRes = await labAPI.tests.getAll();
            setTests(tRes.data || []);
    ...
    const handlePatientSelect = (visit) => {
        ...
        // Match assigned items with available tests
        const assignedItems = visit.assignedItems || [];
        const preSelectedTestIds = tests
            .filter(t => assignedItems.some(item => item.name === t.name))
            .map(t => t.id);
        ...
        setFormData(prev => ({
            ...
            visitId: visit.id,
            selectedTests: preSelectedTestIds,
    ...
    const filteredTests = (tests || []).filter(t =>
        (t.name || '').toLowerCase().includes((formData.searchTerm || '').toLowerCase()) ||
        (t.category || '').toLowerCase().includes((formData.searchTerm || '').toLowerCase())
    );
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-2xl font-black text-white tracking-tighter uppercase italic">
                                Lab <span className="text-primary">Request</span>
                            </h1>
                            <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest mt-1">
                                Clinical investigation & diagnostic order
                            </p>
                        </div>
                        <button onClick={() => navigate('/app/lab/dashboard')} className="p-2 hover:bg-white/5 rounded-full transition-all text-white/40 hover:text-white">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8 animate-fade-in">
                            ...
                            <div className="p-8 rounded-[40px] bg-gradient-to-br from-white/[0.05] to-transparent border border-white/10 shadow-2xl relative overflow-hidden group">
                                ...
                                <div className="relative z-10 flex items-start justify-between">
                                    <div>
                                        <div className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-3">Diagnostic Context</div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-6">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-white/40 uppercase tracking-widest ml-1">Clinical Notes</label>
                                        <textarea
                                            ...
                                            placeholder="Indications for investigation..."
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center px-1">
                                        <label className="text-[10px] font-black text-white/40 uppercase tracking-widest">Select Investigations</label>
                                        ...
                                        <table className="w-full text-[11px]">
                                            <thead className="bg-white/5 sticky top-0 z-10">
                                                <tr>
                                                    ...
                                                    <th className="p-3 text-left font-black text-white/20 uppercase tracking-widest">Test</th>
                                            ...
                                            <tbody className="divide-y divide-white/5 font-bold text-white/80">
                                                {filteredTests.map(test => (
                                                    <tr
                                                        key={test.id}
                                                        className={`hover:bg-white/[0.05] transition-all cursor-pointer ${formData.selectedTests.includes(test.id) ? 'bg-primary/10 text-primary italic' : ''}`}
                                                        onClick={() => toggleTest(test.id)}
                                                    >
                                                        <td className="p-3">
                                                            <div className={`w-4 h-4 rounded border flex items-center justify-center transition-all ${formData.selectedTests.includes(test.id) ? 'bg-primary border-primary' : 'border-white/10 bg-white/5'}`}>
                                                                {formData.selectedTests.includes(test.id) && <Save className="w-2.5 h-2.5 text-white" />}
                                                            </div>
                                                        </td>
                                                        <td className="p-3">{test.name}</td>
                                                        <td className="p-3 text-right">{(test.price || 0).toLocaleString()}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                            ...
                            <div className="p-10 rounded-[50px] bg-white/[0.03] border border-white/10 flex items-center justify-between shadow-inner">
                                <div>
                                    <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em] mb-2 font-mono italic">Investigations Selected</div>
                                    <div className="text-4xl font-black text-white tracking-tighter italic">
                                        {formData.selectedTests.length} <span className="text-xl text-white/40 not-italic ml-1 font-bold tracking-widest">ITEMS</span>
                                    </div>
                                </div>
                                <div className="flex gap-4">
                                    <button
                                        type="button"
                                        onClick={() => navigate('/app/lab/dashboard')}
                                    ...
                                    <button
                                        type="submit"
                                        disabled={loading || (!isPaid && selectedVisit?.patient?.paymentMethod === 'cash') || formData.selectedTests.length === 0}
                                        className={`h-16 px-14 rounded-full text-[11px] font-black uppercase tracking-widest shadow-2xl transition-all italic ${
                                            loading || (!isPaid && selectedVisit?.patient?.paymentMethod === 'cash') || formData.selectedTests.length === 0
                            ...
                            {!isPaid && selectedVisit?.patient?.paymentMethod === 'cash' && (
                                <div className="bg-orange-500/10 border border-orange-500/20 p-6 rounded-[30px] flex items-center gap-6 animate-fade-in relative overflow-hidden">
                                    ...
                                    <p className="text-[11px] font-black text-orange-100/80 uppercase tracking-widest leading-relaxed relative z-10">
                                        Diagnostic processing requires upfront clearance. <br />
